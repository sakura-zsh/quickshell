...
// M3 expressive wave progress/slider, adapted from Clavis Shell
// (Widgets/common/MaterialWaveProgressBar.qml).
// The played part is a live waving line; the rest is a flat track.
// Drag anywhere to seek / set value. Waves animate while `isPlaying`.
Item {
    id: root
    ...
    readonly property bool hovered: seekMa.containsMouse
    ...
    Item {
        ...
        function buildWavePath() {
            ...
            for (let x = padding; x < endX; x += sampleStep) {
                ...
                path += " C " + c1x.toFixed(2) + " " + c1y.toFixed(2) + " " + c2x.toFixed(2) + " " + c2y.toFixed(2) + " " + nextX.toFixed(2) + " " + y1.toFixed(2);
            }
            return path;
        }
        ...
        MouseArea {
            ...
            onReleased: mouse => {
                const clampedX = Math.max(0, Math.min(mouse.x, waveContainer.width));
